...
<body>
   ...
   <script type="text/html" id="addNew_role" >
	...
	<script type="text/html" id="barTool">
  	<a class="layui-btn layui-btn-xs" lay-event="add">添加试卷</a>
    ...
    index: 'lib/index' //主入口模块
      ...
      form.on('submit(addnew1)', function(data){
          ...
          $.ajax({
              url:'text/addText'
              ,type:'POST'
              ,data:{tPid:aId,tName:npName}
              ,success:function(res){
                  if (res.i==0){
					  layer.msg("添加失败,请查看岗位是否存在")
                  }else{
					  layer.msg("添加成功")
					  layer.close(index1)
					  table.reload('inner')
      ...
      table.render({
          ...
          ,cols: [[
              ...
              ,{field:'npName',event:'',  width:200, title: '岗位名'}
              ,{field:'text',  title: '所有试卷',  width:1000, unresize: true,
				  templet: function (d) {
					  var text = '';
					  for (var i in d.texts) {
					  	  var at = d.texts[i];
						  text += '<a href="test/toTest?pid='+at.tid+'" class="layui-btn  layui-btn-sm layui-btn-primary">'+at.tname+'</a>';
					  }
					  return text
				  }
