...
<script type="text/html" id="transfer_role">
...
<script type="text/html" id="barTool">
    <a class="layui-btn layui-btn-xs" lay-event="edit">调动</a>
    ...
    layui.config({
        ...
        table.on("tool(test)", function (obj) {
            ...
            if (layEvent === 'del') {
                ...
                o1 = data.aDepart
                index1 = layer.open({
                    title: "确认调动"+data.name+"吗？",
                    area: ["400px", '275px'],
                    type: 1,
                    shadeClose: true,
                    btn: ["确定", "取消"],
                    skin: 'to-fix-select',
                    content: $('#transfer_role').html(),
                    yes: function (index) {
                        $("#ediit").click();
                    }
                })
        ...
        form.on('submit(addnew1)', function (data) {
            ...
            $.ajax({
                url: 'inner/edit'
                , type: 'POST'
                , data: {aId: aId, name: name1, s1: s1, aStatus: aStatus, p1: npId, p2: nPost, o1: o1, aDepart: aDepart}
                , success: function (res) {
                    if (res.code == 200) {
                        layer.msg("调动成功")
                        layer.close(index1)
