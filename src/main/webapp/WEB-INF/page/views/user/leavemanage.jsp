...
<script type="text/html" id="barTool">
    <a class="layui-btn layui-btn-xs" lay-event="edit">离职</a>
    ...
    layui.config({
        ...
        table.on("tool(test)", function (obj) {
            ...
            if (layEvent === 'del') {
                ...
                layer.confirm("确认让"+data.name+"离职吗？", function (index) {
                    $.post("change/leavesure", {aId: data.aId, uname: data.name, oStatus: data.aStatus}, function (res) {
                        if (res.code == 200) {
                            layer.msg(data.name + "已离职")
                            layer.close(index);
                            table.reload('inner')
                        } else {
                            layer.msg("离职失败")
