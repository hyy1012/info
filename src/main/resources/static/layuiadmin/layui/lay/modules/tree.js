...
layui.define("form", function(e) {
	...
	b.prototype.config = {
		...
		layui.each(r, function(a, r) {
			var l = r.children && r.children.length > 0,
				...
				h = i(['<div data-id="' + r.id + '" class="layui-tree-set' + (r.spread ? " layui-tree-spread" : "") + (r.checked ?
					...
					function() {
						return t.isJump && r.href ? '<a lay-href="' + r.href + '" target="_blank"  class="' + y + '">' + (r.title || r.label ||
		...
		l.children(".layui-tree-btnGroup").on("click", ".layui-icon", function(l) {
			...
			if ("add" == f) {
                $.post("depart/add",{nPid:a.id,nName:'未命名'},function (res) {
                    if (res.code==201){
                        ...
                        var w = r.operate && r.operate(g),
                            N = {};
                        if (N.title = r.text.defaultNodeName, N.id = res.reID, t.tree(e.children("." + v), [N]), r.showLine)
			...
			} else n.confirm('确认删除该节点 "<span style="color: #999;">' + (a.title || "") + '</span>" 吗？', function(ma) {
				console.log(a.id)
				$.post("depart/delete",{id: a.id},function (res) {
					if (res.code == 201) {
						layer.alert("删除失败，请查看当前目录下是否有子目录", {offset: 'auto'})
					} else if(res.code == 200) {
						layer.msg("删除成功", {offset: '400px'})
						if (r.operate && r.operate(g), g.status = "remove", n.close(ma), !e.prev("." + s)[0] && !e.next("." + s)[0] &&
						!e.parent("." + v)[0]) return e.remove(), void t.elem.append(t.elemNone);
						if (e.siblings("." + s).children("." + p)[0]) {
							if (r.showCheckbox) {
								var l = function(e) {
									if (e.parents("." + s)[0]) {
										var ma = e.siblings("." + s).children("." + p),
											n = e.parent("." + v).prev(),
											r = n.find('input[same="layuiTreeCheck"]')[0],
											c = 1,
											d = 0;
										0 == r.checked && (ma.each(function(e, ma) {
											var n = i(ma).find('input[same="layuiTreeCheck"]')[0];
